<template>
  <div class="container-wallet-statistic end-container-wallet"
    v-if="this.$store.state.readyForLoadGraph == 3 && userData.userDataCrypto.length > 0">
    ...
    <div class="box-statistic">
      ...
      <div v-if="onEdit == false">
        ...
        <span class="statistic-data" v-if="userData.userAssetSelected.buyPrice != ''">{{
          userData.userAssetSelected.buyPrice }} $</span>
    ...
    <div class="box-statistic end-box">
      ...
      <div>
        ...
        <span
          v-if="(((userData.userAssetSelected.price-userData.userAssetSelected.buyPrice)/userData.userAssetSelected.buyPrice)*100).toFixed(2) <= 0 && userData.userAssetSelected.buyPrice != ''"
          class="orange">
          {{
        ...
        <span
          v-else-if="(((userData.userAssetSelected.price-userData.userAssetSelected.buyPrice)/userData.userAssetSelected.buyPrice)*100).toFixed(2) >= 0 && userData.userAssetSelected.buyPrice != ''"
          class="green">
          + {{
          (((userData.userAssetSelected.price-userData.userAssetSelected.buyPrice)/userData.userAssetSelected.buyPrice)*100).toFixed(2)
          }} %
                ...
                self.$store.dispatch("loadCryptoPrice", this.$store.getters.getUserListCrypto).then((e) => {
                  if (e) {
                    ...
                    self.$store.dispatch("loadWinLostValue", this.$store.getters.getUserDataCrypto).then((e) => {
                      if (e) {
                        //calcul graph
                        self.$store.dispatch('loadCryptoPriceHistoryMty').then(() => {
